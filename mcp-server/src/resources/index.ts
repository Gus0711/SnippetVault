...
import type { Snippet, SnippetDetail, Collection } from '../types.js';

const RESOURCE_PREFIX = 'snippetvault://';

export function registerResources(server: Server, client: SnippetVaultClient) {
...
function formatSnippetsResource(snippets: Snippet[]): string {
...
function formatCollectionsResource(collections: Collection[]): string {
...
function formatSnippetResource(snippet: SnippetDetail): string {
  const lines = [
    `# ${snippet.title}`,
    '',
    '## Metadata',
